...
class BasicInfo(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.info_id is None:
            salt = "".join([choice(list(list(ascii_letters)+list(digits))) for _ in range(10)])
            self.info_id =  md5(bytes(f"{self.name} {self.address} {self.phone_number} {salt}", "utf-8")).hexdigest()
...
class SectionContent(models.Model):
    ...
    def get_graph_grouped(self):
        return array_split(list({ key:list(value) for key,value in
                groupby(
